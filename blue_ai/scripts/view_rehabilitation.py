from blue_ai.scripts.view_performance import load_dataset
...
if __name__ == "__main__":
    ...
    x = data["step"].unique()
    ax.fill_between(
        x, -1000, 100, where=x < 40, color="skyblue", alpha=0.25, linewidth=0
    )
    ax.fill_between(
        x,
        -1000,
    ...
    ax.fill_between(
        x, -1000, 100, where=x > 80, color="skyblue", alpha=0.25, linewidth=0
    )
    ...
    plt.text(
        x=20, y=0.09, horizontalalignment="center", s="healthy", c="blue", ha="center"
    )
    plt.text(
        x=60,
        y=0.09,
    ...
    plt.text(
        x=30_000, y=avg_results[-1], s="simulated spine loss", c="red", ha="center"
    )
