...
def plot_sample_env(ax):
    ...
    # plot sample env
    env = TransientGoals(img_filename='env1.png', transient_locations=[(4, 1), (5, 4)], transient_obstacles=[(2, 5)],
...
def plot_learning_curves(ax, n_boot=1):
    plt.sca(ax)
...
def plot_goals_per_episode(ax, n_boot=1):
    ...
    plt.ylabel('objects reached per episode')
