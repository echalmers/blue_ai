...
def load_dataset(filename_pattern):
    ...
    results.rename({'dropout': 'dropout (%)'}, axis=1, inplace=True)
    return results
...
def aggregate_goals(type, data):
    ...
    goals.goal_type.replace({'terminal': 'static'}, inplace=True)
    return goals
...
if __name__ == '__main__':
    ...
    plt.title('dropout causes sub-optimal behavior')
    ...
    sns.barplot(data=high_terminal_goals, x='dropout (%)', y='count', hue='goal_type', n_boot=100,
    ...
    plt.title('loss of interest in "hobbies"')
    ...
    sns.barplot(data=high_transient_goals, x='dropout (%)', y='count', hue='goal_type', n_boot=100, palette=['tab:blue', 'tab:green'])
    plt.title('high reward for transient goals')
    plt.ylabel('')
    ax.yaxis.tick_right()
    ...
    transient_count_1 = high_terminal_goals[high_terminal_goals.goal_type == 'transient'][['dropout (%)', 'count']].set_index('dropout (%)')
    transient_count_2 = high_transient_goals[high_transient_goals.goal_type == 'transient'][['dropout (%)', 'count']].set_index('dropout (%)')
    transient_count_2 = transient_count_2.groupby('dropout (%)').mean()
    transient_count_1 = transient_count_1.groupby('dropout (%)').mean()
    plt.arrow(x=0.6, y=transient_count_1.loc[0, 'count'], dx=0, dy=transient_count_2.loc[0, 'count']-transient_count_1.loc[0, 'count'], head_width=0.1, length_includes_head=True, overhang=1)
    plt.plot([0.2, 0.6], [transient_count_1.loc[0, 'count']]*2, c='k')
    plt.arrow(x=1.6, y=transient_count_1.loc[50, 'count'], dx=0, dy=transient_count_2.loc[50, 'count']-transient_count_1.loc[50, 'count'], head_width=0.1, length_includes_head=True, overhang=1)
    plt.plot([1.2, 1.6], [transient_count_1.loc[50, 'count']]*2, c='k')
    plt.annotate(f"+{round((transient_count_2.loc[0, 'count']-transient_count_1.loc[0, 'count'])/transient_count_1.loc[0, 'count']*100)}%",
                 xy=(0.4, transient_count_2.loc[0, 'count']+0.05))
    plt.annotate(f"+{round((transient_count_2.loc[50, 'count']-transient_count_1.loc[50, 'count'])/transient_count_1.loc[50, 'count']*100)}%",
                 xy=(1.4, transient_count_2.loc[50, 'count']+0.05))
