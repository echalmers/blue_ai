...
class PerceivedValuePlotter:
    ...
    def __init__(self, agent_classes=(agent_classes.HealthyAgent, agent_classes.SpineLossDepression)):
        self.env = Image2VecWrapper(
            TransientGoals(img_filename='env1.png', n_transient_obstacles=0, transient_locations=[(4, 5)],
                           agent_start_pos=(4, 4), agent_start_dir=0, render_mode='rgb_array'))
    ...
    def plot_perceived_value(self, ax, palette=('skyblue', 'salmon')):
        ...
        for i in range(len(self.agent_classes)):
            plt.text(x=p.patches[i].get_center()[0], y=0.1, s='↶', fontsize='x-large', fontweight='heavy', ha='center')
            plt.text(x=p.patches[i + len(self.agent_classes)].get_center()[0], y=0.1, s='↑', fontsize='x-large', fontweight='heavy', ha='center')
            plt.text(x=p.patches[i + 2 * len(self.agent_classes)].get_center()[0], y=0.1, s='↷', fontsize='x-large', fontweight='heavy', ha='center')
