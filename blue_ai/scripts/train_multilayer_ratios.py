...
def trial(
    ...
    for agent, env in runs:
        ...
        save_dir = DATA_PATH / "8 Wide Layered Ratios Partial"
        save_dir.mkdir(exist_ok=True)
        hexdigest = xxhash.xxh32_hexdigest(d.to_numpy().tobytes())
        d.write_parquet(
            save_dir
            / f"{current}_{env.get_reward_penalty()}_{agent.file_display_name()}_{hexdigest}.parquet"
        )
