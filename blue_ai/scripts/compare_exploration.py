    ...
    for i in range(20):
        ...
        for j in range(4):
            for agent_class in all_agent_classes:
                ...
                _, agent, _ = load_trial(os.path.join('.', 'data', f'{agent_class.__name__}_{j}.pkl'))

                env = Image2VecWrapper(
                    TransientGoals(agent_start_pos=(randint(1, 5), randint(1, 5)), agent_start_dir=randint(0, 3),
                                   render_mode='rgb_array'))

                for rep in range(10):
                    state, _ = env.reset()
                    while state[1:, :, :].sum() == 0:
                        env = Image2VecWrapper(TransientGoals(agent_start_pos=(randint(1, 5), randint(1, 5)), agent_start_dir=randint(0, 3), render_mode='rgb_array'))
                        state, _ = env.reset()
...
def plot_kl_divergence(ax, **kwargs):
    plt.sca(ax)
    sns.barplot(data=results, x='agent', y='entropy', order=[agent.display_name for agent in all_agent_classes], **kwargs)
