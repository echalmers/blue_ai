...
class DiscountAndCorrelationPlotter:
    ...
    def __init__(
        ...
        for trial in range(N_TRIALS):
            for dataset in [
                ...
                for agent_pos in range(2, 6):
                    ...
                    state, _ = Image2VecWrapper(
                        TransientGoals(
                            img_filename="env1.png",
                            n_transient_goals=0,
                            n_transient_obstacles=0,
                            agent_start_pos=(agent_pos, 6),
                            agent_start_dir=0,
        ...
        self.all_values = pd.DataFrame(
            data=all_values, columns=["trial", "agent", "position", "value"]
        )
        final_values = (
            self.all_values.groupby(["trial", "agent"])
            .last()["value"]
            .reset_index()
            .rename({"value": "final"}, axis=1)
        )
        self.all_values = pd.merge(self.all_values, final_values, on=["trial", "agent"])
        self.all_values["normalized_value"] = (
            self.all_values["value"] / self.all_values["final"]
        )
        self.all_values["estimated discount factor"] = self.all_values[
            "normalized_value"
        ] ** (1 / (5 - self.all_values["position"]))
        self.task_correlations = pd.DataFrame(
            self.task_correlations, columns=["trial", "agent", "correlation"]
        )
    ...
    def plot_value_of_forward(self, ax):
        plt.sca(ax)
        sns.lineplot(
            self.all_values, x="position", y="normalized_value", hue="agent", n_boot=10
        )  # , palette=['skyblue', 'salmon'])
        plt.ylabel(
            "perceived value of moving forward\n(normalized to position 4 value)"
        )
        plt.xticks([1, 2, 3, 4])
    ...
    def plot_inferred_discount(self, ax, **kwargs):
        plt.sca(ax)
        sns.barplot(
            self.all_values[self.all_values["position"] != 5],
            x="agent",
            y="estimated discount factor",
            n_boot=10,
            **kwargs,
        )
